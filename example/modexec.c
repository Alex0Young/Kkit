...
static __init int test_driver_init(void)
{
    ...
    printk(KERN_DEBUG "example2 driver init exec! there result of call_usermodehelper is %d\n", result);
    printk(KERN_DEBUG "example2 driver init exec! the process is \"%s\", pid is %d.\n",current->comm, current->pid);
    return result;
}
